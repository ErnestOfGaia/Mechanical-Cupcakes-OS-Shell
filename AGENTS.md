...
- **Deploy drift is now measurable (L10, 2026-09-11).** Every image carries the commit it was built from (`/health` → `commit`; Penny Post `/version.json`). From anywhere: `node scripts/deploy-drift.mjs` (CURRENT / BEHIND / UNKNOWN / DOWN per app vs the tip of main; exit 0/1/2). On the VPS: `sh scripts/deploy-drift-onbox.sh` (container state, healthcheck, running digest vs GHCR). A daily routine runs the outside half and reports only on drift. Still true: no auto-pull — deploys are manual. `mcos-postcards` crash-looped ~3 months unnoticed; that class of failure now shows as DOWN or RESTARTING within a day.
- ⛔ **`apps/workshop/` is deliberately local-only. Do not wire it into CI or compose.** It is the EOG marketing backoffice's Campaign Workshop, not a gallery exhibit — it reads a private vault of working notes, and its own fail-safe refuses to run in production for that reason. It is finished software that builds and passes its tests; being undeployed is a decision, not a gap. Adding a CI matrix row or a compose service for it needs the W3 storage decision first, which belongs to the Workshop's own product track (SEAM-12). It appears in the gallery as a **placard** at `/workshop` — a card with no app behind it — and that is the whole of MCOS's obligation to it.
